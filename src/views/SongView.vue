  ...
  <div>
    ...
    <section class="w-full mb-8 py-14 text-center text-white relative">
      ...
      <div class="container mx-auto flex items-center">
        ...
        <div class="z-50 text-left ml-8">
          ...
          <div>{{ song.genre }}</div>
          <div class="song-price">{{ $n(5, 'currency', 'ja') }}</div>
        </div>
      </div>
    </section>
    <!-- Form -->
    <section class="container mx-auto mt-6" id="comments">
      <div class="bg-white rounded border border-gray-200 relative flex flex-col">
        ...
        <div class="p-6">
          <div
            class="text-white font-bold text-center p-4 mb-4"
            :class="comment_alert_variant"
            v-if="comment_show_alert"
          >
            {{ comment_alert_msg }}
          </div>
          <vee-form :validation-schema="commentSchema" @submit="createComment" v-if="userLoggedIn">
            <vee-field
              as="textarea"
              name="comment"
          ...
          >
            <option value="1">{{ $t('commentsSort.latest') }}</option>
            <option value="2">{{ $t('commentsSort.oldest') }}</option>
          </select>
        </div>
      </div>
    </section>
...
export default {
  ...
  data() {
    return {
      ...
      comment_alert_msg: 'Please wait! your comment is bieng submitted.',
      comments: [],
      sort: '1'
    }
  },
  ...
  methods: {
    ...
    async createComment(values, { resetForm }) {
      ...
      this.comment_alert_msg = 'Please wait! your comment is bieng submitted.'
      ...
      this.comment_alert_msg = 'Success! Comment added.'
